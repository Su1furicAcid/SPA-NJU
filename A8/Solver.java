...
public class Solver {
    ...
    private PointerFlowGraph taintFlowGraph;
    ...
    private void initialize() {
        ...
        taintFlowGraph = new PointerFlowGraph();
    ...
    private class StmtProcessor implements StmtVisitor<Void> {
        ...
        public Void visit(New stmt) {
            ...
            Context cxtAs = contextSelector.selectHeapContext(csMethod, as);
            CSObj cxtAsObj = csManager.getCSObj(cxtAs, as);
            PointsToSet cxtPts = PointsToSetFactory.make(cxtAsObj);
        ...
        public Void visit(Invoke stmt) {
            ...
            if (callGraph.addEdge(new Edge<>(CallKind.STATIC, csCallSite, csMethod))) {
                ...
                if (varY != null) {
                    ...
                    // If source method
                    Type type = taintAnalysis.isSource(callee);
                    if (type != null) {
                        Obj taintObj = taintAnalysis.makeTaintObj(stmt, type);
                        Context emptyCxt = contextSelector.getEmptyContext();
                        CSObj cxtTaintObj = csManager.getCSObj(emptyCxt, taintObj);
                        PointsToSet cxtTaintPts = PointsToSetFactory.make(cxtTaintObj);
                        workList.addEntry(cxtVarY, cxtTaintPts);
                    }
                }
                // If taint method
                taintAnalysis.doTaintTransfer(stmt, callee, context, null);
            }
            return null;
        }
    }
    ...
    public void addTFGEdge(Pointer source, Pointer target, Type newType) {
        if (taintFlowGraph.addEdge(source, target)) {
            if (!source.getPointsToSet().isEmpty()) {
                ...
                for (CSObj cxtObj : sourcePts) {
                    if (taintAnalysis.isTaint(cxtObj.getObject())) {
                        Obj obj = cxtObj.getObject();
                        Obj newObj = taintAnalysis.makeTaintObj(taintAnalysis.getSourceCall(obj), newType);
                        CSObj cxtNewObj = csManager.getCSObj(contextSelector.getEmptyContext(), newObj);
                        targetPts.addObject(cxtNewObj);
                    }
                }
    ...
    private PointsToSet propagate(Pointer pointer, PointsToSet pointsToSet) {
        ...
        for (CSObj obj : diff) {
            if (taintAnalysis.isTaint(obj.getObject())) {
                newTaints.addObject(obj);
            }
        }
        for (Pointer succPointer : taintFlowGraph.getSuccsOf(pointer)) {
            workList.addEntry(succPointer, newTaints);
        }
        return diff;
    }
        ...
        for (Invoke invoke : recv.getVar().getInvokes()) {
            ...
            if (callGraph.addEdge(new Edge<>(callKind, csCallSite, csMethod))) {
                ...
                if (varY != null) {
                    ...
                    Type type = taintAnalysis.isSource(callee);
                    if (type != null) {
                        Obj taintObj = taintAnalysis.makeTaintObj(invoke, type);
                        Context emptyCxt = contextSelector.getEmptyContext();
                        CSObj cxtTaintObj = csManager.getCSObj(emptyCxt, taintObj);
                        PointsToSet cxtTaintPts = PointsToSetFactory.make(cxtTaintObj);
                        workList.addEntry(cxtVarY, cxtTaintPts);
            ...
            taintAnalysis.doTaintTransfer(invoke, callee, recvContext, recv);
        }
    }
